# C - Recursion

Functions implemented using recursion (no loops, no static variables, no standard library).

## Tasks

- `0-puts_recursion.c` - prints a string followed by a newline
- `1-print_rev_recursion.c` - prints a string in reverse
- `2-strlen_recursion.c` - returns the length of a string
- `3-factorial.c` - returns the factorial of a number
- `4-pow_recursion.c` - returns x raised to the power of y
- `5-sqrt_recursion.c` - returns the natural square root of a number
- `6-is_prime_number.c` - returns 1 if the number is prime, 0 otherwise
